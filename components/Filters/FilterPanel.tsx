		...
		<>
			{isSidebarLayout ? (
				...
				<>
					...
					<button
						type="button"
						className={`${css.filterPanelButton} ${isOpen ? css.filterPanelButtonOpen : ""}`}
						onClick={() => setIsOpen((s) => !s)}
						aria-expanded={isOpen}
					>
						<span>Фільтри</span>
						<svg className={css.filterPanelButtonIcon} width={16} height={16} aria-hidden="true" focusable="false">
							<use href={isOpen ? "/sprite.svg#keyboard_arrow_up" : "/sprite.svg#keyboard_arrow_down"} />
						</svg>
					</button>

					<div className={`${css.filterContainer} ${isOpen ? css.filterContainerOpen : css.filterContainerClosed}`}>
						<Filter options={filters} onClose={() => setIsOpen(false)} variant="dropdown" />
					</div>
				</>
			)}
		</>
